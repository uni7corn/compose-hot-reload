...
internal val Project.hotSnapshotTasks: Future<Iterable<TaskProvider<HotSnapshotTask>>> by projectFuture {
    forAllJvmCompilations { compilation ->
        compilation.hotSnapshotTask.await()
    }
}

internal val KotlinCompilation<*>.hotSnapshotTask: Future<TaskProvider<HotSnapshotTask>> by future {
    val hotRuntimeFiles = this.hotRuntimeFiles
    project.tasks.register(this.hotSnapshotTaskName, HotSnapshotTask::class.java) { task ->
